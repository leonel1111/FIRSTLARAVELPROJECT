...
<body>
    <header class="header-bar mb-3">
        <div class="container d-flex flex-column flex-md-row align-items-center p-3">
            ...
            {{-- conditional header where it will depend if the user is logged in or logged out  --}}
            @auth
                <div class="flex-row my-3 my-md-0">
                    ...
                    <form action="/logout" method="POST" class="d-inline">
                        {{-- CSRF protection is like a special lock on a diary that prevents someone from secretly making you do things on a website you didn't intend to do. --}}
                        @csrf
                        <button class="btn btn-sm btn-secondary">Sign Out</button>
                    </form>
                </div>
                {{-- this will show up if the user is not logged in --}}
            @else
