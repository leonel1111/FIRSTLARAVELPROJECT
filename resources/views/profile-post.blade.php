...
<div class="container py-md-5 container--narrow">
    ...
    <div class="profile-nav nav nav-tabs pt-2 mb-4">
        <a href="#" class="profile-nav-link nav-item nav-link active">Posts: {{$userpostcount}}</a>
        <a href="#" class="profile-nav-link nav-item nav-link">Followers: 3</a>
        <a href="#" class="profile-nav-link nav-item nav-link">Following: 2</a>
    </div>

    <div class="list-group">
    @foreach($userprofileposts as $singularpost)
    <a href="/post/{{$singularpost->id}}" class="list-group-item list-group-item-action">
        ...
        <strong>{{$singularpost->title}}</strong> on {{$singularpost->created_at->format('n/j/Y')}}
    </a>
    @endforeach
